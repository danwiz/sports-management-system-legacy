...
char Main_Menu_Display()
{
   ...
   g.display(30,13,GREEN,WHITE," Member Management");
   ...
   do
   {
       ...
       g.Go();//unhides the cursor
  	     if(pos==1)
         {
           ...
           g.display(28,13,BLACK,LIGHTGRAY,"   Member Management   ");
               ...
               g.display(28,13,BLACK,LIGHTGRAY,"   Member Management   ");
                  ...
                  g.display(28,12,BLACK,LIGHTGRAY,"   Match Management     ");
           			...
           			g.display(28,13,YELLOW,LIGHTGRAY,"\20");
 		     			g.display(30,13,WHITE,GREEN," MEMBER MANAGEMENT ");
           			g.display(50,13,YELLOW,LIGHTGRAY,"\21");

			  			g.display(28,14,BLACK,LIGHTGRAY,"   Exit Program                    ");
           			g.display(1,25,BLACK,LIGHTGRAY, " ADDS,EDITS & DISPLAYS MEMBER INFOMATION: NAME, ADRESSS FAUCLTY  ETC...         ");
           			gotoxy(46, 16);
   ...
   do
    {
      ...
      id_already_exist = mread.SearchFile(id); //searches the user file for this ID number
    ...
    g.SetScreen(); //clears the screen and set the background color to black
   ...
   do
   {
    ...
    if(MatchType==1)     //if the chosen for creation was soccer ask for thw added info
    {
      ...
      cout<<endl<<"Enter Number of Throws Given This Match :  ";
    	cin>>throws;
      socmatch newSoc(id,officials,location,time,date,team1,team2,winner,loser,mvp,draw,penalties,corners,throws);
      ...
      if(matches)
      {
      	matches<<"Match ID          : S"<<newSoc.getid()<<endl;
         matches<<"Match Competitors :  "<<newSoc.getTeam1().data()<<"  vs  "<<newSoc.getTeam2().data()<<endl;
         matches<<"Match Location    :  "<<newSoc.getLocation().data()<<endl;
         matches<<"Match Date        :  "<<newSoc.getTime().data()<<"     "<<newSoc.getDate().data()<<endl;
         matches<<"Match Officials   :  "<<newSoc.getOfficials().data()<<endl;
         matches<<"Match Statistics     "<<endl;
         matches<<"Penalties         :  "<<newSoc.getPKicks()<<endl;
         matches<<"Throws            :  "<<newSoc.getThrows()<<endl;
         matches<<"Corners           :  "<<newSoc.getCorners()<<endl;
         matches<<"Match Winner      :  "<<newSoc.getWinner().data()<<endl<<endl<<endl<<endl;
      }
    	...
    	if(MatchType==2)
      {
      	...
      	cout<<endl<<"Enter Number of Free Passes Won This Match :  ";
    		cin>>fpasses;
         netmatch newNet(id,officials,location,time,date,team1,team2,winner,loser,mvp,draw,penalties,fpasses);
      	if(nwrite.WriteToFile(newNet.getid(),newNet)) //writes the new socmatch to file to the file
     		{
       		g.display(25,23,YELLOW,BLACK,"Database updated successfully....");
        ...
        if(matches)
        {
        		matches<<"Match ID          : N"<<newNet.getid()<<endl;
            matches<<"Match Competitors :  "<<newNet.getTeam1().data()<<"  vs  "<<newNet.getTeam2().data()<<endl;
            matches<<"Match Location    :  "<<newNet.getLocation().data()<<endl;
            matches<<"Match Date        :  "<<newNet.getTime().data()<<"     "<<newTrack.getDate().data()<<endl;
            matches<<"Match Officials   :  "<<newNet.getOfficials().data()<<endl;
            matches<<"Match Statistics     "<<endl;
            matches<<"Penalties         :  "<<newNet.getPenalties()<<endl;
            matches<<"Free Passes       :  "<<newNet.getFPasses()<<endl;
            matches<<"Match Winner      :  "<<newNet.getWinner().data()<<endl<<endl<<endl<<endl;
        }
      }
      else
      	if(MatchType==3)
         {
            ...
            cout<<endl<<"Enter the Race fastest Time :  ";
    			cin>>ftime;
         	trackmatch newTrack(id,officials,location,time,date,team1,team2,winner,loser,mvp,draw,race_title,fstarts,ftime);
      		if(twrite.WriteToFile(newTrack.getid(),newTrack)) //writes the new socmatch to file to the file
     			{
       			g.display(25,23,YELLOW,BLACK,"Database updated successfully....");
            ...
            if(matches)
    			{
    				matches<<"Match ID          : T"<<newTrack.getid()<<endl;
               matches<<"Match Competitors :  "<<newTrack.getTeam1().data()<<"  vs  "<<newTrack.getTeam2().data()<<endl;
               matches<<"Race Title        :  "<<newTrack.getTitle().data()<<endl;
               matches<<"Match Location    :  "<<newTrack.getLocation().data()<<endl;
               matches<<"Match Date        :  "<<newTrack.getTime().data()<<"     "<<newTrack.getDate().data()<<endl;
               matches<<"Match Officials   :  "<<newTrack.getOfficials().data()<<endl;
               matches<<"Match Statistics     "<<endl;
               matches<<"False Starts      :  "<<newTrack.getFStarts()<<endl;
               matches<<"Fastest Time      :  "<<newTrack.getFTime()<<endl;
               matches<<"Match Winner      :  "<<newTrack.getWinner().data()<<endl<<endl<<endl<<endl;
...
void Credits()
{
   ...
   gotoxy(31, 19);cprintf("We Obviously Have Too Much Time on Our Hands");
...
void ExitPro()
{
    GRAPHIX g;
     ...
     OnTeams();
	  ...
	  g.display(10,8,LIGHTGRAY,BLACK,"Open File \'Print_Match_Info \' To Print Info about Matches");
